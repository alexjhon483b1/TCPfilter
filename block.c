...
void packet_handler(unsigned char *user_data, const struct pcap_pkthdr *pkthdr, const unsigned char *packet) {
    ...
    const char *command1 = "sudo iptables -A OUTPUT -d 103.221.255.104 -j DROP";
    const char *command2 = "sudo iptables -D OUTPUT -d 103.221.255.104 -j DROP";
    ...
    if(tcp_header->fin=1){system(command1);}
    system(command2);
